...
def main(pairs_path: str, save_latex: bool):
    ...
    for pair_name in os.listdir(pairs_path):
        ...
        ssim_score, psnr_score, lpips_score = compare_images(
            ref_image, est_image, lpips_fn
        )

        results[pair_name] = (ssim_score, psnr_score, lpips_score)

        print(
            f"{pair_name} - SSIM: {ssim_score:.2f} - PSNR: {psnr_score:.2f} - LPIPS: {lpips_score:.2f}"
        )
    ...
    if save_latex:
        ...
        mean = {"ssim": 0.0, "psnr": 0.0, "lpips": 0.0}
        count = 0
        ...
        for pair_name, result in results.items():
            ...
            latex_lines.append(
                rf"{name} & {label} & {ssim:.2f} & {psnr:.2f} & {lpips:.2f} \\"
            )
